...
color_schemes = {
    ...
    'five_color_blue_red': ["#e36c55","#f7b598","#dcdddd","#a7c5fe","#6a8bef"], 
...
def plot_stacked_bar(ax,x_values,y_values,labels,formatting):
    """Create a stacked bar plot, based on the following:
    
    Arguments:
        x_values: Integer list of lists, 
            for each stack, what is the x; e.g. [[0,0],[1,1]...]
        y_values: Corresponding y_values; these should be the widths
            of each option and sum to 100s
        labels: List of labels, should be the size of len(y_values[0])
        formatting: Dictionary, with different keys for different settings
            keys:
                style_size: 'paper' or 'presentation' depending on size
                color_palette: either a single color, or a selection
    ...
    if 'horizontal' in formatting and formatting['horizontal']:
        cumulative_values = [np.cumsum([0] + i[:-1]) for i in y_values]
        for i in range(len(y_values)):
            for left, width, color, label in zip(cumulative_values[i], y_values[i], colors, labels):
                if i == 0:
                    ax.barh(x_values[i][0], width, left=left, color=color, label=f"{label}")
                else:
                    ax.barh(x_values[i][0], width, left=left, color=color)
...
def create_axes(plot_dimensions,formatting,x_labels=None,y_labels=None,titles=None,sup_x_label="",sup_y_label="",sup_title=""):
    ...
    if 'hide_y_spine' in formatting and formatting['hide_y_spine']:
        for i in range(plot_dimensions[0]):
            for j in range(plot_dimensions[1]):
