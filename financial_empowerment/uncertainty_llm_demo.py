...
extract_sys_prompt = "You are an assistant that extracts relevant personal and financial information from user inputs. \
                      Your task is to present the information in a structured format without making any assumptions or judgments about the data.\
                      Focus on identifying key informations."

# LLM API call to extract eligibility information from user input
def call_llm_extract(user_input, pdf_text):
    prompt = f"Extract the relevant personal information from the following user input and present it in a structured format. \
        Pay attention to the attributes or requirements that are mentioned in the text :{pdf_text}. These extracted information will \
         be used to determine if the user is eligible for certain government benefit. Here is just an example of the structure. \
            You should extract additional attributes or personal information if you see fit: \n" \
...
update_sys_prompt = "You are an assistant that updates previously extracted personal and financial information based on user corrections. \
                    Your task is to update only the specific information the user mentions and retain the rest of the information as it was. "
...
check_sys_prompt = "You are an assistant that evaluates eligibility for various government benefits based on the user’s personal and financial information. \
                   Your task is to assess user eligibility for different benefits. For each benefit, clearly explain the reasoning behind the classification, \
                    specifying which requirements are met and which are not. If some information is missing or uncertain, indicate how this impacts eligibility. \
                    Ensure the reasoning is easy to understand but provides enough detail for the user to know why they fall into a specific category."
...
def call_llm_check_eligibility(extracted_info, pdf_text):
    prompt = f"Please read the following text extracted from a pdf file :{pdf_text} containing benefits and eligibility criteria. \n" \
             ...
             f"Determine if the user is 100% eligible, likely eligible, maybe eligible, or not eligible for any benefits. \
                Pay close attention to eligiblity details and user information. make sure that your answer is accurate\
                Provide the reasoning for each in the following structure:\n" \
             f"- 100% eligible:\n[List of benefits]\n" \
